...
export default async function PacientesPage({ searchParams }: PageProps) {
  ...
  let query = supabase
    .from('patients')
    .select('id, first_name, last_name, dni, email, phone, date_of_birth, created_at', { count: 'exact' })
    .order('last_name')
    .range(from, to)
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            ...
            {q?.trim() ? ` que coinciden con "${q}"` : ' en total'}
          </p>
        </div>
        <Link
        ...
        {!patients || patients.length === 0 ? (
          ...
          <>
            ...
            <Pagination
              ...
              total={total}
              pageSize={PAGE_SIZE}
              hrefPrev={pageHref(q, page - 1)}
              hrefNext={pageHref(q, page + 1)}
            />
          </>
        )}
      </div>
